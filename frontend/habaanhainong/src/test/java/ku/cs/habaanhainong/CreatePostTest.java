...
public class CreatePostTest {
    ...
    void testCreatePost() {
        ...
        driver.get("http://localhost:" + port + "/post-new");

        titleField.sendKeys("หาบ้าน");
        descriptionField.sendKeys("รายละเอียด");
        nameField.sendKeys("น้องมะลิ");
        typeField.sendKeys("แมว");
        sexField.sendKeys("เพศผู้");
        breedField.sendKeys("ไม่ทราบ");
        addressField.sendKeys("แถวนี้");
        submitButton.click();

        WebElement title = driver. findElement(By.xpath("//*[text()='หาบ้าน']"));
        WebElement description = driver. findElement(By.xpath("//*[text()='รายละเอียด']"));

        assertEquals("หาบ้าน", title.getText());
        assertEquals("รายละเอียด", description.getText());
    }
}
